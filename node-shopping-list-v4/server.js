...
app.put('/recipes/:id', jsonParser, (req, res) => {
  const fields = ['name', 'id', 'ingredients'];
  for (let i=0; i < fields.length; i++) {
    if (!(fields[i] in req.body )) {
      const missingField = `${fields[i]} is missing from the reqest`;
      console.log(missingField);
      return res.status(400).send(missingField);
    }
  }
  ...
  if (req.params.id !== req.body.id) {
    ...
    console.log(idNoMatch);
    return res.status(400).send(idNoMatch);
  }

  console.log(`Updating recipes with the id ${req.params.id}`);
  console.log(`${req.params.id} ${req.params.body} ${req.params.ingredients}`);
